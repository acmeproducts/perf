...
for (const DEV of (process.env.DEVICES || 'Pixel 7,Desktop Chrome').split(',')) test(`e2e ${DEV}`, async ({ browser }) => {
  ...
  for (let k = 0; k < 4; k++) {
    ...
    // Watch every frame from the tap until the tapped image shows: any frame where Focus is up and a
    // different image is visible is a flash of the previous image.
