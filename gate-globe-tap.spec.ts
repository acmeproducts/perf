...
for (const dev of ['Pixel 7', 'Desktop Chrome'] as const) test(`${dev}: globe tap opens the tapped image`, async ({ browser }) => {
  ...
  await page.evaluate(() => {
    ...
    document.addEventListener('pointerdown', e => { log('down:' + ((typeof SG.cardIdAt === 'function' ? SG.cardIdAt(e.clientX, e.clientY) : (document.elementFromPoint(e.clientX, e.clientY)?.closest('.spatial-gallery__card') as HTMLElement | null)?.dataset.fileId) ?? '-')); }, true);
  ...
  for (let trial = 0; trial < 20; trial++) {
    ...
    const t = await page.evaluate((trial) => {
      ...
      const G = (window as any).SpatialGallery; const id = typeof G.cardIdAt === 'function' ? G.cardIdAt(cx, cy) : (document.elementFromPoint(cx, cy)?.closest('.spatial-gallery__card') as HTMLElement | null)?.dataset.fileId;
  ...
  const fx = await page.evaluate(() => { const g = (window as any).SpatialGallery; g.velocityX = 0; g.velocityY = 0; return typeof g.cardIdAt === 'function' ? g.cardIdAt(innerWidth/2, innerHeight/2) : (document.elementFromPoint(innerWidth/2, innerHeight/2)?.closest('.spatial-gallery__card') as HTMLElement | null)?.dataset.fileId; });
