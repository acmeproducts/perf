...
test.describe('Pace-timeout is recorded, never swallowed (found via isolated POC)', () => {
  test('a card whose fetch never settles is counted as a real, visible error after 20s, not silently dropped', async ({ page }) => {
    await page.clock.install();
    await page.route('https://pace-hang.test/**', () => { /* never fulfill -- permanently hung request */ });
    await page.goto(uiUrl);
    await page.waitForFunction(() => !!(window as any).__orbitalAppState);
    const before = await page.evaluate(() => {
      const g = window as any;
      ...
      state.providerType = 'test-provider';
      state.imageFiles = [
        { id: 'hang', name: 'hang', stack: 'in', stackSequence: 1, metadataStatus: 'loaded', thumbnails: { medium: { url: 'https://pace-hang.test/hang.svg' } } },
      ];
      state.currentFolder = { id: 'pace-test', name: 'pace-test' };
      state.currentStack = 'in';
      state.currentStackPosition = 0;
      state.stacks = { in: [], out: [], priority: [], trash: [] };
      g.SharedImageResources.clear();
      g.Core.initializeStacks();
      g.SpatialGallery.open({ stackName: 'in', fileId: 'hang' });
      return {
        errorsBefore: g.SharedImageResources.instrumentation.imageErrorCount,
        timeoutsBefore: g.SharedImageResources.instrumentation.paceTimeoutCount || 0,
      };
    });
    ...
    const after = await page.evaluate(() => {
      const resources = (window as any).SharedImageResources;
      return {
        errorsAfter: resources.instrumentation.imageErrorCount,
        timeoutsAfter: resources.instrumentation.paceTimeoutCount || 0,
      };
    });
    expect(before.timeoutsBefore).toBe(0);
    expect(after.timeoutsAfter).toBe(1);
    expect(after.errorsAfter).toBe(before.errorsBefore + 1);
  });
});
