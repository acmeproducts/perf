...
# --- VERSION: static identity first, so a later runtime issue can never masquerade as an old build. ---
...
# --- FOCUS CHROME IS THE ONE STANDARD ---
# Keep legacy surface controls available to old controller references, but make them permanently nonvisual.
...
body.orbital-table-standard .pill-counter {
  display:none !important;
}
'''
replace_once(focus_css_anchor, focus_css, 'canonical Focus chrome CSS')

# Existing v1.7 duplicate conditional block is now redundant; remove it to avoid competing rules.
regex_once(
...
# Explore is a Focus-chrome surface from entry through medium/full inspection.
replace_once(
    '''                this.elements.root.hidden = false;\n                this.buildCards();''',
    '''                this.elements.root.hidden = false;\n                document.body.classList.add('orbital-explore-active');\n                this.buildCards();''',
    'Explore active class open'
)
replace_once(
    '''                this.elements.root.hidden = true;\n                this.loadGeneration++;''',
    '''                this.elements.root.hidden = true;\n                document.body.classList.remove('orbital-explore-active');\n                this.loadGeneration++;''',
    'Explore active class close'
)

# Keep actual Focus controls bound to the selected Explore image.
replace_once(
...
# On Explore entry, synchronize Focus chrome immediately even before a thumbnail is tapped.
replace_once(
    ...
    'Explore initial Focus sync'
)

# The Focus stack selector must also work while Explore is on the sphere, not only in medium/full inspection.
replace_once(
...
# --- EXPLORE FLOATING SELECTOR: fixed geometry, drag the value/body; no click-to-morph. ---
replace_once(
    ...
    '''        .spatial-gallery__adjust { display: block; width: 34px; height: 30px; padding: 0; border: 1px solid rgba(255,255,255,.25); border-radius: 999px; color: #fff; background: rgba(30,41,59,.95); font: 700 20px/1 system-ui; cursor: pointer; }\n        .spatial-gallery__value { cursor: grab; }\n        .spatial-gallery__controls.dragging .spatial-gallery__value { cursor: grabbing; }''',
    'Explore selector fixed geometry'
)
regex_once(
    r'''\s*this\.elements\.controls\?\.querySelectorAll\('\.spatial-gallery__value'\)\.forEach\(button => button\.addEventListener\('click', event => \{\s*event\.stopPropagation\(\); button\.closest\('\.spatial-gallery__stepper'\)\.classList\.toggle\('expanded'\);\s*\}\)\);''',
    '',
    'remove selector expand toggle'
)
replace_once(
    "                if (event.target.closest('button')) return;",
    "                if (event.target.closest('.spatial-gallery__adjust')) return;",
    'selector drag from value/body'
)

# --- TABLE: fake heart is removed at construction, not merely hidden. ---
regex_once(
    r'''\s*const favorite=document\.createElement\('button'\);favorite\.id='photo-table-favorite';favorite\.type='button';favorite\.className='photo-table__favorite';favorite\.setAttribute\('aria-label','Favorite armed image'\);favorite\.textContent='♥';root\.appendChild\(favorite\);''',
    '',
    'remove fake Table heart creation'
)

# Existing listener is harmless with no element, but remove it so there is only one favorite path: Focus.
regex_once(
    r'''\s*document\.getElementById\('photo-table-favorite'\)\?\.addEventListener\('click',async\(\)=>\{.*?\}\);''',
    '',
    'remove fake Table favorite handler'
)

# Sync the real Focus chrome whenever a Table photo becomes armed/touched.
replace_once(
...
# YES/MAYBE/NO only; Inbox stays a scattered source and can never become a visible/physical target.
# v1.7 already hides Inbox; make physics ignore hidden targets too.
replace_once(
    "    const targets = tableTargetGeometryV14();",
    "    const targets = tableTargetGeometryV14().filter(target => target.el.dataset.tablePile !== 'in' && !target.el.hidden);",
    'Table physics excludes Inbox'
)

# --- TABLE TRAILS: exact Sort comet visual, not just the same class name. ---
replace_once(
    '''        .gesture-layer .comet-trail {''',
    '''        .gesture-layer .comet-trail,\n        .photo-table .comet-trail {''',
    'Sort comet CSS shared with Table'
)

# Make Table target labels and semantics unambiguous everywhere that remains user-visible.
...
# Footer runtime log identifies exactly what is under test.
text = text.replace(
    "Table scatters Inbox into YES/MAYBE/NO; target tap opens Grid; Sort comet trails reused; image-image collisions disabled.",
    "v1.8 focus-standard: Explore and Table use real Focus chrome; Explore selector fixed/draggable; Table Inbox scatters to draggable YES/MAYBE/NO; exact Sort trails; no image-image collisions."
)

# Final assertions are requirements, not implementation guesses.
required = [
...
if text == original:
    raise SystemExit('v1.8 editor made no changes')
